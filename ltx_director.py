...
class LTXDirector(io.ComfyNode):
    ...
    def execute(cls, model, clip, global_prompt, duration_frames, duration_seconds,
        ...
        # explicit start_image is given, decode its LAST latent frame here and use it as this
        # chunk's frame-0 start keyframe, so the chunk continues from where the last one ended —
        # no separate Latent Tail to Image node needed. This produces exactly the same single
        # start keyframe as wiring `LatentTailToImage -> start_image` by hand.
        if start_image is None and prev_latent is not None:
            ...
            tail = prev_samples[:, :, -1:].contiguous()  # last latent frame only
        ...
        try:
            ...
            # Log with a full traceback — a swallowed error here previously made chaining
            # silently do nothing (the start keyframe never got built).
            log.error("[PromptRelay] Could not build guide_data: %s", e, exc_info=True)
